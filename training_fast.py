...
from model_fast import batched_forward, create_attention_mask, init_params
...
def loss_fn(params, x, y):
    pred_y = batched_forward(x, params, n_outer_blocks, n_blocks, mask, start_index_for_x_wq)
...
def test_a_batch(batch, params):
    ...
    while batch.shape[-1] != (seq_len + shrink_factor):
        out = batched_forward(batch, params, n_outer_blocks, n_blocks, jnp.zeros(batch.shape[-1] // shrink_factor), start_index_for_x_wq = -1)
...
def train_and_test(train_loader, test_loader, params, opt_state):
    for epoch in range(66):
        ...
        for batch in tqdm(train_loader):
            batch = batch[0]
            x, y = jnp.array(batch[:, :-shrink_factor]), jnp.array(batch[:, start_index_for_x_wq + 1:])
            loss, params, opt_state = train_step(params, opt_state, x, y)
            total_loss += loss
        print(f"Averge epoch loss: {total_loss / len(train_loader)}")
...
start_index_for_x_wq = original_n_unmasked // shrink_factor # in shrunken space

assert seq_len % shrink_factor == 0, "Sequence length must be divisible by the shrink factor"
assert original_n_unmasked % shrink_factor == 0, "Unmasked elements (should) be divisible by the shrinyk factor"
...
# with this configuration, mask is only created for the future elements (not the context, unmasked elements)
